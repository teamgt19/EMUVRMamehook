...
MAME_API int __stdcall init_mame(int clientid, const char * name, MAME_START start, MAME_STOP stop, MAME_COPYDATA copydata, MAME_UPDATESTATE updatestate);
MAME_API int __stdcall close_mame(void);
MAME_API LPCSTR __stdcall map_id_to_outname(WPARAM id);
...
/* BOOL CALLBACK EnumWindowsProc(HWND hWnd, LPARAM lParam)
{
	...
	if (strncmp(titlebar, (const char *)lParam, strlen((const char *) lParam)) == 0)
	{
		mame_hwnd = hWnd;
		return(FALSE);
	}
...
{
	BOOL brc;
	LPTSTR titlebar = "MAME:";
	brc = EnumWindows( EnumWindowsProc, (LPARAM) titlebar);
} */
